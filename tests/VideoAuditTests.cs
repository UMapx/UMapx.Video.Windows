...
public class VideoAuditTests
{
    ...
    public void MjpegFramesSurviveArbitraryTransportChunkBoundaries(int chunk)
    {
        ...
        Assert.Equal(2,colors.Count);VideoTestImages.Pixel(Color.Red,colors[0],4,false);VideoTestImages.Pixel(Color.Blue,colors[1],4,false);
    }
    ...
    {
        using var bitmap=VideoTestImages.Pattern(12,10,false);var received=new TaskCompletionSource<Size>(TaskCreationOptions.RunContinuationsAsynchronously);var depthReceived=new TaskCompletionSource<int>(TaskCreationOptions.RunContinuationsAsynchronously);
