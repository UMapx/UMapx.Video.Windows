namespace UMapx.Video.DirectShow
{
    ...
    public class VideoCaptureDevice : IVideoSource
    {
        ...
        private WindowsVideoCapabilities videoResolution = null;
        private WindowsVideoCapabilities snapshotResolution = null;
        ...
        private WindowsVideoCapabilities[] videoCapabilities;
        private WindowsVideoCapabilities[] snapshotCapabilities;
        ...
        private static readonly Dictionary<string, WindowsVideoCapabilities[]> cacheVideoCapabilities = new Dictionary<string, WindowsVideoCapabilities[]>();
        private static readonly Dictionary<string, WindowsVideoCapabilities[]> cacheSnapshotCapabilities = new Dictionary<string, WindowsVideoCapabilities[]>();
        ...
        /// </remarks>
        /// 
        public WindowsVideoCapabilities VideoResolution
        {
        ...
        /// </remarks>
        /// 
        public WindowsVideoCapabilities SnapshotResolution
        {
        ...
        /// </remarks>
        /// 
        public WindowsVideoCapabilities[] VideoCapabilities
        {
            get
            {
                ...
                return videoCapabilities ?? (new WindowsVideoCapabilities[0]);
            }
        }
        ...
        /// <seealso cref="ProvideSnapshots"/>
        /// 
        public WindowsVideoCapabilities[] SnapshotCapabilities
        {
            get
            {
                ...
                // don't return null even capabilities are not provided for some reason
                return snapshotCapabilities ?? (new WindowsVideoCapabilities[0]);
            }
        }
        ...
        {
            this.deviceMoniker = deviceMoniker;
            _ = MediaSubType.ConvertFrom(pixelFormat);
        ...
        private void WorkerThread(bool runGraph)
        {
            ...
            try
            {
                ...
                using AMMediaType mediaType = new AMMediaType
                {
                    ...
                    SubType = MediaSubType.ConvertFrom(pixelFormat)
                };
                ...
                {
                    snapshotCapabilities = new WindowsVideoCapabilities[0];
                }
                ...
                if (runGraph)
                {
                    ...
                    {
                        var header = RgbVideoFormat.Read(mediaType, pixelFormat);
                        videoGrabber.Width = header.Width;
                        videoGrabber.Height = header.Height;
                    ...
                    if ((isSnapshotSupported) && (provideSnapshots))
                    {
                        ...
                        {
                            var header = RgbVideoFormat.Read(mediaType, pixelFormat);
                            snapshotGrabber.Width = header.Width;
                            snapshotGrabber.Height = header.Height;
            ...
            finally
            {
                ...
                videoControl = null;
                mediaEvent = null;
                if (pinStillImage != null) Marshal.ReleaseComObject(pinStillImage);
                pinStillImage = null;
                crossbar = null;
        ...
        private void SetResolution(IAMStreamConfig streamConfig, WindowsVideoCapabilities resolution = null)
        {
            ...
            for (int i = 0; i < capabilitiesCount; i++)
            {
                try
                {
                    WindowsVideoCapabilities vc = new WindowsVideoCapabilities(streamConfig, i);
        ...
        private void GetPinCapabilitiesAndConfigureSizeAndRate(ICaptureGraphBuilder2 graphBuilder, IBaseFilter baseFilter,
            Guid pinCategory, WindowsVideoCapabilities resolutionToSet, ref WindowsVideoCapabilities[] capabilities)
        {
                ...
                if (streamConfig != null)
                {
                    if (capabilities == null)
                    {
                        try
                        {
                            ...
                            capabilities = UMapx.Video.DirectShow.WindowsVideoCapabilities.FromStreamConfig(streamConfig);
                        }
                        catch
                        {
                        }
            ...
            if (capabilities == null)
            {
                capabilities = new WindowsVideoCapabilities[0];
            }
        }
        ...
        private void OnSnapshotFrame(Bitmap image)
        {
            ...
            {
                if ((!worker.IsStopping) && (SnapshotFrame != null))
                    VideoSourceCallbacks.Invoke(() => SnapshotFrame?.Invoke(this, new NewFrameEventArgs(image)));
            }
        }
        ...
        private class Grabber : ISampleGrabberCB
        {
            ...
            public int BufferCB(double sampleTime, IntPtr buffer, int bufferLen)
            {
                try
                {
                    if (!parent.worker.IsStopping && (snapshotMode ? parent.SnapshotFrame != null : parent.NewFrame != null))
                    {
                        using (Bitmap image = BitmapFrame.Copy(buffer, bufferLen, width, height, pixelFormat))
                        {
                            if (snapshotMode) parent.OnSnapshotFrame(image);
                            else parent.OnNewFrame(image);
                        }
                    }
                }
